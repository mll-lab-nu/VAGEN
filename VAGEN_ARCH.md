...
| Algorithm layer — token / turn / bi-level | ⚠️ | token-level ✅, GRPO ✅ (both layout-independent, on `TrajectoryView`); turn-level still the original 338-line implementation; **bi-level not written** |
