...
| `bi_level_gae_varlam` | same, plus a second `lam` at turn boundaries | `lam_low == lam` reproduces token-level exactly; `lam_low == 1` reproduces turn-level at each turn's first token. Both limits are pinned by tests. Needs `+algorithm.lam_low=…` — hydra's `+`, because verl's `AlgoConfig` has no such field. Not defaulted: its limits are the other two estimators, so a missing value would run one of them under this name. |
| `bi_level_gae` | same | **the published VAGEN algorithm, as released** (ported from `compute_bi_level_gae_advantage_return`, commit 4076507 in this repo's history — not from the prose, since §4.2 and Algorithm 2 disagree about the overwrite). Outer turn-level GAE anchored at each turn's *last* token with `+algorithm.high_level_gamma`; the turn's return is written back as that token's reward; the inner token pass zeroes both bootstrap and accumulator at every turn end, so the last token receives exactly `A_turn` and each turn's chain is independent. Kept so `bi_level_gae_varlam`'s three corrections can be measured rather than argued. Needs the turn's reward on the turn's last token. |
