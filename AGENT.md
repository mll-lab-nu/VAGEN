...
| `bi_level_gae_paper` | same | **the published VAGEN algorithm, as released** (ported from `compute_bi_level_gae_advantage_return`, commit 4076507 in this repo's history — not from the prose, since §4.2 and Algorithm 2 disagree about the overwrite). Outer turn-level GAE anchored at each turn's *last* token with `+algorithm.high_level_gamma`; the turn's return is written back as that token's reward; the inner token pass zeroes both bootstrap and accumulator at every turn end, so the last token receives exactly `A_turn` and each turn's chain is independent. Kept so `bi_level_gae`'s three corrections can be measured rather than argued. Needs the turn's reward on the turn's last token. |
