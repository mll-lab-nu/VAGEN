...
* an estimator with one reward slot per turn (``removed_estimator_gae_paper`` reads a turn's reward
...
def test_a_typo_in_the_estimator_name_does_not_silently_lump():
    ...
    fuzzy-matched. Reading ``removed_estimator_gae_papper`` as the paper estimator would silently
    change where every reward in the run is paid."""
    assert resolve_reward_placement(_cfg("removed_estimator_gae_papper"), "auto") == "per_span"
...
def test_an_explicit_setting_overrides_auto():
    ...
    assert resolve_reward_placement(_cfg("removed_estimator_gae_paper"), "per_span") == "per_span"
