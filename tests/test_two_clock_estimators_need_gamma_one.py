"""``bi_level_gae`` runs two clocks, and they only agree at ``gamma == 1``.
...
def test_a_discounted_bi_level_run_is_refused_at_startup():
    with pytest.raises(ValueError, match=r"only defined at algorithm.gamma=1\.0"):
        _Trainer("bi_level_gae", 0.99)._vagen_check_estimator_is_undiscounted()
...
def test_gamma_one_passes():
    _Trainer("bi_level_gae", 1.0)._vagen_check_estimator_is_undiscounted()
