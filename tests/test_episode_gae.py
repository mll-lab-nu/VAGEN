"""``episode_gae`` -- the vanilla baseline the multi-turn estimators are measured against.
...
def test_every_token_is_handed_the_whole_episode_total():
    ...
    """
    _, ret, mask = _call("episode_gae", _concat(), gamma=1.0, lam=1.0)
...
def test_where_the_reward_sat_makes_no_difference():
    ...
    spread = _call("episode_gae", _concat((0.0, MID, 0.0, END)), lam=0.7)
    up_front = _call("episode_gae", _concat((TOTAL, 0.0, 0.0, 0.0)), lam=0.7)
    at_the_end = _call("episode_gae", _concat((0.0, 0.0, 0.0, TOTAL)), lam=0.7)
...
def test_the_episode_total_is_conserved():
    ...
    either way."""
    _, lumped, mask = _call("episode_gae", _concat(), gamma=1.0, lam=1.0)
...
def test_only_the_critic_distinguishes_two_tokens():
    ...
    adv, _, mask = _call("episode_gae", _concat(values=values), gamma=1.0, lam=1.0)
...
def test_lambda_is_honoured():
    ...
    strict, _, mask = _call("episode_gae", _concat(), lam=1.0)
    bootstrapped, _, _ = _call("episode_gae", _concat(), lam=0.3)
...
def test_under_concat_it_is_exactly_verls_gae():
    ...
    _, mine, mask = _call("episode_gae", already_lumped, gamma=1.0, lam=0.9)
...
def test_verls_gae_would_credit_every_row_under_no_concat():
    ...
    _, mine, mask = _call("episode_gae", _split(), gamma=1.0, lam=1.0)
...
def test_the_two_layouts_agree():
    ...
    a_concat, r_concat, m_concat = _call("episode_gae", _concat(), lam=0.8)
    a_split, r_split, m_split = _call("episode_gae", _split(), lam=0.8)
...
def test_declares_what_the_trainer_needs_to_know():
    assert "episode_gae" in TRAJECTORY_ESTIMATORS, "must stitch an episode's rows together"
    assert spans_rows("episode_gae") is True
    assert needs_critic("episode_gae") is True
    # It supervises every model-output token, so a value_mask would be wrong for it.
    assert needs_value_mask("episode_gae") is False


def test_is_not_a_two_clock_estimator():
    """★ Unlike ``removed_estimator_gae_varlam`` this runs one recursion on one clock, so ``gamma < 1``
    ...
    assert "episode_gae" not in UNDISCOUNTED_ESTIMATORS
    assert requires_undiscounted("episode_gae") is False
...
def test_publishes_turn_id_like_every_other_packing_estimator():
    ...
    get_adv_estimator_fn("episode_gae")(batch=batch, non_tensor_batch=non_tensor, config=_Cfg())
...
def test_a_shorter_trajectory_still_gets_its_own_total():
    ...
    _, ret, mask = _call("episode_gae", layout, gamma=1.0, lam=1.0)
...
def test_a_trajectory_with_no_model_output_does_not_blow_up():
    ...
    adv, ret, mask = _call("episode_gae", layout)
