...
the variable-lambda ``removed_estimator_gae_varlam``, and the wrong one for the paper's nested removed_estimator
...
class StateRewardWrapper:
    ...
    def _place(self, scored: dict, outcome: float, token_ids, tokenizer) -> list[float]:
        ...
        ``turn_end``
            ...
            chain has one reward slot per turn -- ``removed_estimator_gae`` reads a turn's
        ...
        ``per_span``
            ...
            the variable-lambda ``removed_estimator_gae_varlam``: measured -28% variance at lam 0.9 and
