...
```python
from vagen.envs._common.remote.handler import BaseGymHandler
...
obs, reward, done, info = await env.step(action)   # the GymImageEnv 4-tuple;
                                                  # envs/_common/adapter.py widens it
await env.close()
```
...
envs/
├── remote/
│   └── remote_env.py         # selectable client implementation
└── _common/remote/
    ├── service.py            # FastAPI server
    ├── handler.py            # handler/session contracts
    ├── multipart_codec.py    # image + JSON transport
    └── README.md
```
