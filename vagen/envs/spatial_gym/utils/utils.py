...
def parse_llm_response(text: str, enable_think: bool = True) -> Tuple[str, str, bool]:
    """Parse the shared free-think or answer-only protocol."""
    if not isinstance(text, str):
        return "", "", False
    sections = (
        parse_free_think_sections(text, allow_native_answer=True)
        if enable_think
        else parse_answer_sections(text)
    )
    return sections.reasoning, sections.answer, sections.format_correct
...
def format_llm_output(reasoning_content: str, answer_content: str, enable_think: bool = True) -> str:
    """Format output using the repository-wide response protocol."""
    if enable_think:
        return f"<think>{reasoning_content}</think><answer>{answer_content}</answer>"
...
if __name__ == "__main__":
    ...
    llm_response = "<think>I will move then observe.</think><answer>Actions: [JumpTo(table), Observe()]</answer>"
