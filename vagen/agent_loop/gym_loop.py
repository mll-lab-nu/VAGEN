...
from vagen.core.env_adapter import GymEnvAdapter, _accepts_response
...
def resolve_reward_placement(config, configured: str = "auto") -> str:
    ...
    ``auto`` is the default for the same reason ``lam_low`` has none on ``removed_estimator_gae_varlam``:
