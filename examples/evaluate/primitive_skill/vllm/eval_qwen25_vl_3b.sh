#!/bin/bash
# Evaluate on PrimitiveSkill against a local vLLM server.
#
# setup.py calls vLLM "the verified default" and the two engines are mutually exclusive
# extras, but primitive_skill shipped only an sglang launcher -- so following the install instructions and then the
# evaluation instructions left you with an engine no example here could drive.
#
#   MODEL_PATH=/path/to/model bash examples/evaluate/primitive_skill/vllm/eval_qwen25_vl_3b.sh
#
...
# Not 8000: vagen/envs/primitive_skill/serve.py defaults to 8000, and this eval needs it.
PORT="${PORT:-8311}"
TP_SIZE="${TP_SIZE:-1}"
...
# primitive_skill's environment runs in its own HTTP server, and the eval config points at
